...
!(async () => {
  ...
  if (!res) { 
    res = await getAuthorShareCode('https://ghproxy.inoyna12.workers.dev/https://raw.githubusercontent.com/inoyna12/updateTeam/master/shareCodes/jxhb.json') 
    if (!res) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/inoyna12/updateTeam@master/shareCodes/jxhb.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('https://inoyna12.coding.net/p/updateteam/d/updateTeam/git/raw/master/shareCodes/jxhb.json')
    }
  }
